...
Install RoomCast as a PWA
-------------------------
- The controller now serves a Web App Manifest (`/static/manifest.webmanifest`) and service worker (`/sw.js`). Open the RoomCast UI in Chrome, Edge, Safari, or Firefox (Android) and use the browser's install prompt to add it to your dock/home screen.
- Assets are cached for offline launch, and when a new build is published the UI shows a persistent "Update now" banner. Tap it to refresh immediately; otherwise the service worker updates automatically on the next visit.
