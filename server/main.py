...
from services.snapcast_service import SnapcastService
...
snapcast_service = SnapcastService(
    ...
    normalize_percent=lambda *args, **kwargs: _normalize_percent(*args, **kwargs),
    is_rpc_method_not_found_error=is_rpc_method_not_found_error,
)
...
channels_service = ChannelsService(
    ...
    sanitize_channel_color=_sanitize_channel_color,
    normalize_spotify_source_id=_normalize_spotify_source_id,
    require_spotify_provider=require_spotify_provider,
    ...
    channels_by_id=lambda: channels_by_id,
    normalize_radio_state=_normalize_radio_state,
    default_radio_state=_default_radio_state,
    ensure_radio_state=_ensure_radio_state,
    abs_channel_slots=ABS_CHANNEL_SLOTS,
    require_audiobookshelf_provider=require_audiobookshelf_provider,
    normalize_abs_state=_normalize_abs_state,
    save_channels=save_channels,
...
app.include_router(
    create_channels_router(
        ...
        update_channel_metadata=lambda channel_id, updates: channels_service.update_channel_metadata(channel_id, updates),
...
app.include_router(
    create_streams_router(
        ...
        summarize_snapserver_status=snapcast_service.summarize_snapserver_status,
        public_snap_client=snapcast_service.public_snap_client,
        get_webrtc_relay=lambda: webrtc_relay,
...
nodes_store = NodesStore(
    ...
    transient_node_fields=TRANSIENT_NODE_FIELDS,
    normalize_section_name=_normalize_section_name,
    normalize_node_url=_normalize_node_url,
    default_eq_state=default_eq_state,
    default_eq_preset=DEFAULT_EQ_PRESET,
    normalize_percent=_normalize_percent,
...
def load_nodes() -> None:
    ...
    nodes, sections = nodes_store.load()
...
async def _startup_events() -> None:
    ...
    if WEBRTC_ENABLED:
        webrtc_relay = WebAudioRelay(
            ...
            assign_stream=snapcast_service.assign_webrtc_stream,
